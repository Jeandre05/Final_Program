"""Page which the user first uses, the user is given a choice.
The user either login or sign-up and is then taken to the homepage."""
...
####### Functions #######
class User:
    """Creates class where email, id, password,name and dob are saved onto user object.
     This user object will be used to organise my variables under one name. 
     This way i can change and shift them around in an organised way."""
...
def error_login():
    """ This function is used when the user enters the wrong details in the login function. 
    Then this error prompt is shown. """
    messagebox.showerror("Login Error", "Error: Login or password is incorrect!")

def login():
    """This function is used for when the user tries to login.
   The function acesses the database and sees if the data in entries is present in database.
   If not the prorgam will give an error or if present the prorgam takes you to the homepage."""
    ...
    try:
        ...
        messagebox.showerror(
            "Login Error", "Error: The user does not yet have sign-up details."
        )
...
#Signup function
def sign_up():
    """The signup function lets the user enter their details into entries. 
    Validation checks the values entered and tells the user to re-enter them if they do not fit the parameters. 
    The correct values are then entered into users database.
    Next a workouts table is created with all the preset workouts and also a flag variable is created to check if someone has logged in before."""
    ...
    if not email or not password or not dob or not name:
        messagebox.showerror(
            "Data Error", "Error: Please enter all fields before sign-up!"
        )
        return
    #Email pattern taken from an external source 
    ...
    if not re.match(email_pattern, email):
        ...
        return
    #DOB pattern taken from an external source
    ...
    if not re.match(dob_pattern, dob):
        messagebox.showerror(
            ...
            "Error: Invalid date of birth format.\
             ...
             Has to be withing 100 years of 2022",
        )
        return
    ...
    cursor.execute(
        "INSERT INTO users (email, password, name, dob) VALUES (?, ?, ?, ?)",
    ...
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS workouts (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, type TEXT, reps INTEGER, weight INTEGER, sets INTEGER)"
    )

    # Execute SQL statement to insert workouts into table
    cursor.executemany(
        "INSERT INTO workouts (name, type, reps, weight, sets) VALUES (?, ?, ?, ?, ?)",
        workouts,
    )
...
login_button = customtkinter.CTkButton(
    top_frame, text="Login", command=login, fg_color=("black", "black")
)
...
password_entry_signup = ttk.Entry(
    bottom_frame, textvariable=password_entred_signup, show="*"
)
...
signup_button = customtkinter.CTkButton(
    bottom_frame, text="Sign-Up", command=sign_up, fg_color=("black", "black")
)
