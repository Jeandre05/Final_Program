"""This datapage will give the user the capability to change their data.
Data such as their, email, password, dob and most significantly their PR values.
Changing your PR values will change the weight the workouts will display.
At the start of program weights are preset but can be customised this way."""
####### Imports #######
...
####### Functions #######
# Function that changes details on button press
def change_details():
    """This function takes the data for email, password, dob, PR for legs, chest and arms. 
    From the entered data it changes the repsective values in users database to the new values user entered.
    This function then runs the change weights function."""
...
def change_weights():
    """For the weight of the workouts the the PR value entered is devided by a workout variable.
    Each workout has an unique workout varibale which customised the large weight appropriately.
    An if statement is then run for each type, running from one spot in list to another.
    This new weight is put into the workoouts database.
    Entry fields are then cleared to tell the user the change has occured."""
